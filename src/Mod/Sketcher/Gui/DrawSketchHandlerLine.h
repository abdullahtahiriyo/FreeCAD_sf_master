...
{
    friend DrawSketchHandlerLineController;  // allow DrawSketchHandlerRectangleBase specialisations
...
private:
    ...
    virtual void executeCommands() override
    {
        try {
            ...
            Gui::NotifyError(sketchgui, "Failed to add line: %s\n", e.what());
...
template<>
auto DrawSketchHandlerLineController::getState(int labelindex) const
{
    switch (labelindex) {
...
template<>
void DrawSketchHandlerLineController::configureToolWidget()
{
...
template<>
void DrawSketchHandlerLineController::doEnforceControlParameters(Base::Vector2d& onSketchPos)
{
...
template<>
void DrawSketchHandlerLineController::adaptParameters(Base::Vector2d onSketchPos)
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (!onViewParameters[OnViewParameter::First]->isSet) {
...
template<>
void DrawSketchHandlerLineController::doChangeDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (onViewParameters[OnViewParameter::First]->isSet
...
// autoconstraints)
template<>
void DrawSketchHandlerLineController::addConstraints()
{
