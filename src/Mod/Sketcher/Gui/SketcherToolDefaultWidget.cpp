...
void SketcherToolDefaultWidget::reset()
{
    ...
    for (int i = 0; i < nCheckbox; i++) {
        ...
        setCheckboxPrefEntry(i, "DummySetting");
    }
...
void SketcherToolDefaultWidget::checkBoxTS1_toggled(bool val) {
    if (!blockParameterSlots) {
        if (isCheckboxPref(Checkbox::FirstBox)) {
...
void SketcherToolDefaultWidget::checkBoxTS2_toggled(bool val) {
    if (!blockParameterSlots) {
        if (isCheckboxPref(Checkbox::SecondBox)) {
...
void SketcherToolDefaultWidget::checkBoxTS3_toggled(bool val) {
    if (!blockParameterSlots) {
        if (isCheckboxPref(Checkbox::ThirdBox)) {
...
void SketcherToolDefaultWidget::checkBoxTS4_toggled(bool val) {
    if (!blockParameterSlots) {
        if (isCheckboxPref(Checkbox::FourthBox)) {
...
void SketcherToolDefaultWidget::initNCheckbox(int ncheckbox)
{
    Base::StateLocker lock(blockParameterSlots, true);

    isPref.resize(ncheckbox);

    std::fill(isPref.begin(), isPref.end(), false);
...
bool SketcherToolDefaultWidget::isCheckboxPref(int checkboxindex)
{
    if (checkboxindex < nCheckbox) {
        return isPref[checkboxindex];
    }

    THROWM(Base::IndexError, "ToolWidget checkbox index out of range");
}

void SketcherToolDefaultWidget::setCheckboxPref(int checkboxindex, std::string prefEntry)
{
    if (checkboxindex < nCheckbox) {
        isPref[checkboxindex] = true;
        setCheckboxPrefEntry(checkboxindex, prefEntry);
    }
}

void SketcherToolDefaultWidget::setCheckboxPrefEntry(int checkboxindex, std::string prefEntry)
{
    if (checkboxindex < nCheckbox) {
