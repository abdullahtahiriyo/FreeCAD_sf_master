...
using DrawSketchHandlerRectangleBase =
    DrawSketchControllableHandler<DrawSketchHandlerRectangleController>;


class DrawSketchHandlerRectangle: public DrawSketchHandlerRectangleBase
{
    friend DrawSketchHandlerRectangleController;  // allow DrawSketchHandlerRectangleBase
                                                  // specialisations access
                                                  // DrawSketchHandlerRectangle private members
...
private:
    ...
    virtual void executeCommands() override
    {
        try {
            ...
            THROWM(Base::RuntimeError,
                   "Tool execution aborted\n")  // This prevents constraints from being applied on
                                                // non existing geometry
        }
    ...
    virtual QPixmap getToolIcon() const override
    {
        return Gui::BitmapFactory().pixmap("Sketcher_CreateRectangle");
    }

    virtual QString getToolWidgetText() const override
    {
        return QString(QObject::tr("Rectangle parameters"));
    }

    virtual void angleSnappingControl() override
    {
        if ((constructionMethod() == ConstructionMethod::ThreePoints)
...
template<>
auto DrawSketchHandlerRectangleController::getState(int labelindex) const
{
...
template<>
void DrawSketchHandlerRectangleController::configureToolWidget()
{
...
template<>
void DrawSketchHandlerRectangleController::adaptDrawingToCheckboxChange(int checkboxindex,
                                                                        bool value)
{
...
template<>
void DrawSketchHandlerRectangleController::doEnforceControlParameters(Base::Vector2d& onSketchPos)
{
        ...
        case SelectMode::SeekSecond: {
            if (handler->constructionMethod() == ConstructionMethod::Diagonal) {
                if (onViewParameters[OnViewParameter::Third]->isSet) {
                    onSketchPos.x =
                        handler->corner1.x + onViewParameters[OnViewParameter::Third]->getValue();
                }
                if (onViewParameters[OnViewParameter::Fourth]->isSet) {
                    onSketchPos.y =
                        handler->corner1.y + onViewParameters[OnViewParameter::Fourth]->getValue();
                }
            }
            else if (handler->constructionMethod() == ConstructionMethod::CenterAndCorner) {
                if (onViewParameters[OnViewParameter::Third]->isSet) {
                    onSketchPos.x = handler->center.x
                        + onViewParameters[OnViewParameter::Third]->getValue() / 2;
                }
                if (onViewParameters[OnViewParameter::Fourth]->isSet) {
                    onSketchPos.y = handler->center.y
                        + onViewParameters[OnViewParameter::Fourth]->getValue() / 2;
                }
            }
            else if (handler->constructionMethod() == ConstructionMethod::ThreePoints) {
                Base::Vector2d v = onSketchPos - handler->corner1;
                double length = v.Length();
                if (onViewParameters[OnViewParameter::Third]->isSet) {
                    onSketchPos = handler->corner1
                        + v * onViewParameters[OnViewParameter::Third]->getValue() / length;
            ...
            if (handler->constructionMethod() == ConstructionMethod::Diagonal
                ...
                if (handler->roundCorners) {
                    if (onViewParameters[OnViewParameter::Fifth]->isSet) {
                        Base::Vector2d vecL = (handler->corner2 - handler->corner1)
                            / (handler->corner2 - handler->corner1).Length();
                        onSketchPos = handler->corner1
                    ...
                    if (onViewParameters[OnViewParameter::Sixth]->isSet) {
                        ...
                        Base::Vector2d u = (handler->corner2 - handler->corner1)
                            / (handler->corner2 - handler->corner1).Length();
                        Base::Vector2d v = (handler->corner4 - handler->corner1)
                            / (handler->corner4 - handler->corner1).Length();
                        onSketchPos = handler->corner1 - u * thickness - v * thickness;
                    }
                }
            }
            else if (handler->constructionMethod() == ConstructionMethod::ThreePoints) {
                Base::Vector2d v = onSketchPos - handler->corner2Initial;
                double width = v.Length();
                if (onViewParameters[OnViewParameter::Fifth]->isSet) {
                    onSketchPos = handler->corner2Initial
                        + v * onViewParameters[OnViewParameter::Fifth]->getValue() / width;
                    width = onViewParameters[OnViewParameter::Fifth]->getValue();
                }
                if (onViewParameters[OnViewParameter::Sixth]->isSet) {
                    double angle123 = (handler->corner2Initial - handler->corner1).Angle() + M_PI
                    ...
                    if (handler->side
                        != handler->getPointSideOfVector(onSketchPos,
                                                         ...
                                                         handler->corner1)) {
                        angle123 = (handler->corner2Initial - handler->corner1).Angle() + M_PI
                            + onViewParameters[OnViewParameter::Sixth]->getValue() * M_PI / 180;
                    }
                ...
                if (onViewParameters[OnViewParameter::Sixth]->isSet) {
                    ...
                    double angle = (handler->center - handler->corner1).Angle() - (c - a);
                    if (handler->side
                        != handler->getPointSideOfVector(onSketchPos,
                                                         handler->corner3 - handler->corner1,
                                                         handler->corner1)) {
                        angle = (handler->center - handler->corner1).Angle() + (c - a);
                    }
            ...
            if (handler->constructionMethod() == ConstructionMethod::Diagonal
                ...
                if (handler->roundCorners) {
                    if (onViewParameters[OnViewParameter::Seventh]->isSet) {
                        ...
                        Base::Vector2d vecL = (handler->corner2Initial - handler->corner1)
                            / (handler->corner2Initial - handler->corner1).Length();
...
template<>
void DrawSketchHandlerRectangleController::adaptParameters(Base::Vector2d onSketchPos)
{
...
template<>
void DrawSketchHandlerRectangleController::doChangeDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (onViewParameters[OnViewParameter::First]->isSet
...
template<>
void DrawSketchHandlerRectangleController::addConstraints()
{
