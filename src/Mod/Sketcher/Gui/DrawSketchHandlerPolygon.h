...
using DrawSketchHandlerPolygonBase =
    DrawSketchControllableHandler<DrawSketchHandlerPolygonController>;


class DrawSketchHandlerPolygon: public DrawSketchHandlerPolygonBase
{
    friend DrawSketchHandlerPolygonController;
...
template<>
auto DrawSketchHandlerPolygonController::getState(int labelindex) const
{
    switch (labelindex) {
...
template<>
void DrawSketchHandlerPolygonController::firstKeyShortcut()
{
...
template<>
void DrawSketchHandlerPolygonController::secondKeyShortcut()
{
    auto value = toolWidget->getParameter(WParameter::First);
    if (value > 3.0) {
        toolWidget->setParameter(OnViewParameter::First, value - 1);
...
template<>
void DrawSketchHandlerPolygonController::configureToolWidget()
{
...
template<>
void DrawSketchHandlerPolygonController::adaptDrawingToParameterChange(int parameterindex,
                                                                       double value)
{
...
template<>
void DrawSketchHandlerPolygonController::doEnforceControlParameters(Base::Vector2d& onSketchPos)
{
        ...
        case SelectMode::SeekSecond: {
            ...
            if (onViewParameters[OnViewParameter::Third]->isSet) {
                ...
                if (length < Precision::Confusion()) {
                    ...
                    onSketchPos = handler->centerPoint + v * length / v.Length();
                }
            }
...
template<>
void DrawSketchHandlerPolygonController::adaptParameters(Base::Vector2d onSketchPos)
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (!onViewParameters[OnViewParameter::First]->isSet) {
...
template<>
void DrawSketchHandlerPolygonController::doChangeDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            if (onViewParameters[OnViewParameter::First]->isSet
...
template<>
void DrawSketchHandlerPolygonController::addConstraints()
{
