...
void TextGeometryExtension::copyAttributes(Part::GeometryExtension * cpy) const
{
    ...
    static_cast<TextGeometryExtension *>(cpy)->shapeOverride = this->shapeOverride;
}

void TextGeometryExtension::restoreAttributes(Base::XMLReader &reader)
{
    Part::GeometryPersistenceExtension::restoreAttributes(reader);
    shapeOverride = reader.getAttributeAsInteger("shapeOverride");
}
...
void TextGeometryExtension::saveAttributes(Base::Writer &writer) const
{
    ...
    writer.Stream() << "\" shapeOverride=\"" << shapeOverride;
}
...
TopoDS_Shape TextGeometryExtension::toShape() const
{
    if(geometry) {
        if(shapeOverride)
            ...
            return geometry->toShape();
    }

    THROWM(Base::ValueError, "TextGeometryExtension does not have valid pointer to the Geometry object.");
}
