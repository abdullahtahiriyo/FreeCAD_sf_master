            ...
            for(const auto & c : constraints) {
                ...
                if(c->Type != Coincident) {
                    ...
                    if(focuscoincidentfound != axisgeoid2parabolageoid.end()) {
                        ...
                        // endpoints need to be reversed
                        auto geo = static_cast<Part::GeomLineSegment *>(geometries[focuscoincidentfound->first]);
                        geo->setPoints(geo->getEndPoint(), geo->getStartPoint());
