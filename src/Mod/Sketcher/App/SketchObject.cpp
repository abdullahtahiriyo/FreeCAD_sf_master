...
void SketchObject::updateGeoHistory() {
    ...
    for(int i=0;i<(int)geos.size();++i) {
        ...
        geoHistory->update(pstart,geof->getId());
        if(pstart!=pend)
            geoHistory->update(pend,-geof->getId());
    }
...
void SketchObject::generateId(Part::Geometry *geo) {
    ...
    if(!geoHistoryLevel) {
        geof->setId(++geoLastId);
        geoMap[geof->getId()] = (long)Geometry.getSize();
        return;
    }
...
void SketchObject::onChanged(const App::Property* prop)
{
    if (prop == &Geometry) {
        ...
        for(long i=0;i<(long)vals.size();++i) {
            ...
            if(!geof->getId())
                geof->setId(++geoLastId);
            else if(geof->getId() > geoLastId)
                geoLastId = geof->getId();
            bool warned = false;
            while(!geoMap.insert(std::make_pair(geof->getId(),i)).second) {
                if(!warned) {
                    warned = true;
                    FC_WARN("duplicate geometry id " << geof->getId());
                }
                geof->setId(++geoLastId);
            }
        }
...
std::string SketchObject::checkSubName(const char *sub) const{
    ...
    if(geo && GeometryFacade::getFacade(geo)->getId() == id) {
        char sep;
        int posId = none;
        std::ostringstream ss;
...
std::string SketchObject::convertSubName(const char *shapetype) const{
    ...
    if(geoId>=0)
        ss << 'g' << std::hex << geof->getId();
    else
        ss << 'e' << std::hex << shapetype+1;
    if(posId!=none)
        ss << 'v' << std::hex << posId;
    ss << '.' << shapetype;
    return ss.str();
...
bool SketchExport::update() {
    ...
    while(edgeList.size()) {
        ...
        std::deque<int> hashes;
        std::vector<TopoDS_Edge> edges{0};//Part::sort_Edges2(Precision::Confusion(),edgeList,&hashes);
