...
SketchObject::SketchObject()
{
    ...
    ADD_PROPERTY_TYPE(ExternalGeometry,(0,0),"Sketch",(App::PropertyType)(App::Prop_None),"Sketch external geometry links");
    ADD_PROPERTY_TYPE(ExternalGeo,     (0)  ,"Sketch",(App::PropertyType)(App::Prop_None),"Sketch external geometry");

    allowOtherBody = true;
    allowUnaligned = true;

    const std::vector< Part::Geometry * > &vals = getExternalGeometry();
    ...
    newVals.push_back(HLine);
    newVals.push_back(VLine);

    ExternalGeo.setValues(newVals);

    for (std::vector<Part::Geometry *>::iterator it=newVals.begin(); it != newVals.end(); ++it)
        if (*it) delete *it;
...
bool SketchObject::convertToNURBS(int GeoId)
{
    if (GeoId > getHighestCurveIndex() ||
        (GeoId < 0 && -GeoId > static_cast<int>(ExternalGeo.getSize())) ||
        GeoId == -1 || GeoId == -2)
        return false;
...
int SketchObject::carbonCopy(App::DocumentObject * pObj, bool construction)
{
    ...
    if(psObj->ExternalGeometry.getSize()>0) { 
        ...
        addLinksToExternalGeometry( sObjects, sSubElements);
    }
...
{
    assert(ExternalGeometry.getSize() == (ExternalGeo.getSize()-2));
...
int SketchObject::delExternal(int ExtGeoId)
{
    ...
    newVals.erase(newVals.begin()+ExtGeoId+2);
...
int SketchObject::delAllExternal()
{
    ...
    newVals.erase(newVals.begin()+2,newVals.end());
...
const Part::Geometry* SketchObject::getGeometry(int GeoId) const
{
    ...
    else if (GeoId <= -1 && -GeoId <= int(ExternalGeo.getSize()))
        return ExternalGeo.getValues()[-GeoId-1];
...
bool SketchObject::validateExternalLinks(void)
{
    ...
    // In legacy behaviour there is no ExternalGeo property stored, only ExternalGeometry property
    // so:
    // - a rebuild of external geometry is mandatory
    //
    // In non-legacy mode if the array sizes are consistent, then:
    // - if a link is invalid, it is possible to preserve the defining state (normal/construction)
    //
    // In non-legacy mode if the array sizes are inconsistent, then:
    // - a rebuild of external geometry is mandatory and the defining state is lost
    
    bool legacy = isLegacyExternalGeometry();
...
{
    if (!isExternalAllowed(Object->getDocument(), Object))
        return -1;
    ...
    for (size_t i = 0  ;  i < Objects.size()  ;  ++i){
        if (Objects[i] == Object   &&   std::string(SubElement) == SubElements[i]){
    ...
    SubElements.push_back(std::string(SubElement));
    ...
    try {
        AddLinkToArray(Object, SubElement, newVals);
    }
    catch (const Base::Exception& e) {
        Base::Console().Error("%s\n", e.what());
        // revert to original values
...
void SketchObject::AddLinkToArray( DocumentObject* &Object, 
...
void SketchObject::AddLinksToArray( std::vector<DocumentObject*> &Objects, 
                                    std::vector<std::string> &SubElements,
                                    std::vector< Part::Geometry * > &newVals)
{
                    ...
                    if (fabs(dnormal.Angle(snormal) - M_PI_2) < Precision::Confusion()) {
                        ...
                        if (builder.IsDone()) {
                            ...
                            if (curve.GetType() == GeomAbs_Line) {
                                newVals.push_back(projectLine(curve, gPlane, invPlm));
                            }
                        }
                            ...
                            for (xp.Init(projShape, TopAbs_EDGE); xp.More(); xp.Next()) {
                                ...
                                if (projCurve.GetType() == GeomAbs_Line) {
                                    ...
                                    if (Base::Distance(p1,p2) < Precision::Confusion()) {
                                        ...
                                        Part::GeomPoint* point = new Part::GeomPoint(p);
                                        point->Construction = true;
                                        newVals.push_back(point);
                                    }
                                    else {
                                        Part::GeomLineSegment* line = new Part::GeomLineSegment();
                                        line->setPoints(p1,p2);
                                        line->Construction = true;
                                        newVals.push_back(line);
                                    }
                                }
...
std::vector<Part::Geometry*> SketchObject::getCompleteGeometry(void) const
{
    ...
    vals.insert(vals.end(), ExternalGeo.getValues().rbegin(), ExternalGeo.getValues().rend()); // in reverse order
    return vals;
}
...
{
    // ExternalGeo has H and V Axes
    // ExternalGeometry has the actual number of imported links
    //
    // Note: if ExternalGeometry==0, legacy=true 
    bool legacy = isLegacyExternalGeometry();

    try {
        if(!validateExternalLinks()) {
            // if there were external broken links, external geometry was already rebuilt

            // if not, if we are in legacy mode we need to rebuild it anyway as we did not have stored ExternalGeo property
            if(legacy)
                rebuildExternalGeometry(); 
    ...
    for(std::size_t ic = 0; ic<newVals.size(); ic++){//ic = index of constraint
        ...
        for(int ig=1; ig<=3; ig++){//cycle through constraint.first, second, third
            ...
            if ( geoId <= GeoEnum::RefExt &&
                ...
                Part::Geometry* g = this->ExternalGeo.getValues()[-geoId-1];
