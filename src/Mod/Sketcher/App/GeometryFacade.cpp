...
GeometryFacade::GeometryFacade(Part::Geometry * geometry)
: Geo(geometry)
{
    if(!geometry->hasExtension(SketchGeometryExtension::getClassTypeId())) {
        ...
        Base::Console().Warning("%s\nSketcher Geometry without Extension: %s \n", boost::uuids::to_string(geometry->getTag()).c_str());
    }

    SketchGeoExtension =
        std::static_pointer_cast<SketchGeometryExtension>(
            (geometry->getExtension(SketchGeometryExtension::getClassTypeId())).lock()
        );
}
