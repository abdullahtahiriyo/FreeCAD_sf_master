...
void PropertyContainer::changedPropertyName(Base::XMLReader &reader, const char * TypeName, const char *PropName)
{
    ...
    for (int i=0 ;i<Cnt ;i++) {
        ...
        try {
            ...
            // name matches but not the type
                ...
                changedPropertyType(reader, TypeName.c_str(), prop);
            }
            // name doesn't match, the sub-class then has to know
            // if the property has been renamed or removed
            else {
                changedPropertyName(reader, TypeName.c_str(), PropName.c_str());
            }
